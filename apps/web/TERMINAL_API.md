...
Web 工作台「终端」tab（`apps/web/src/components/TerminalView.tsx`）与服务端之间的实时终端协议。服务端按本文档实现 `GET /api/terminal/:sessionId` 的 WebSocket 端点，把 botmux PTY 会话的屏幕流推给浏览器，并把浏览器的键盘输入回写给 PTY。

...
export function NewSessionModal({ open, onClose, onOpenAgentSetup, onCreated, agents, capabilities }: NewSessionModalProps) {
  ...
  if (!open) return null;
  if (agents.length === 0) return <div className="ui-overlay fixed inset-0 z-20 grid place-items-center bg-slate-950/35 p-4 backdrop-blur-[2px]"><div role="dialog" aria-modal="true" aria-label="需要先准备 Agent" className="ui-dialog w-full max-w-md rounded-2xl border border-slate-200 bg-[var(--paper)] p-5 shadow-[0_24px_80px_rgba(15,23,42,.22)]"><div className="flex items-center"><h2 className="text-base font-semibold">先准备一个 Agent</h2><span className="ml-auto"><IconButton label="关闭" onClick={onClose}><X size={16}/></IconButton></span></div><p className="mt-4 text-sm leading-6 text-slate-600">Dockmux 没有检测到可用的 Agent CLI。安装并登录 Codex、Claude Code 等 CLI 后重启 Dockmux，再回来创建任务。</p><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={onClose} className="min-h-10 rounded-lg border border-slate-300 px-3 text-sm font-medium">稍后再说</button><button type="button" onClick={onOpenAgentSetup ?? onClose} className="min-h-10 rounded-lg bg-slate-900 px-3 text-sm font-semibold text-white">查看添加方法</button></div></div></div>;
  if (legacyPtyUnsupported) return <div className="ui-overlay fixed inset-0 z-20 grid place-items-center bg-slate-950/35 p-4 backdrop-blur-[2px]"><div role="dialog" aria-modal="true" aria-label="无法创建旧 PTY 任务" className="ui-dialog w-full max-w-md rounded-2xl border border-slate-200 bg-[var(--paper)] p-5 shadow-[0_24px_80px_rgba(15,23,42,.22)]"><div className="flex items-center"><h2 className="text-base font-semibold">无法创建任务</h2><span className="ml-auto"><IconButton label="关闭" onClick={onClose}><X size={16}/></IconButton></span></div><p role="alert" className="mt-4 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs leading-5 text-red-700">旧 PTY 兼容协议无法提供可靠的权限控制或交互审批，请改用 ACP 或 PTY CLI Agent。</p><div className="mt-5 flex justify-end"><button type="button" disabled className="rounded-lg bg-teal-700 px-3.5 py-2 text-xs font-semibold text-white opacity-50">创建并执行</button></div></div></div>;
  const submitDisabled = create.isPending || !goal.trim() || (permissionMode === 'full-trust' && !fullTrustConfirmed);
  return <div className="ui-overlay fixed inset-0 z-20 grid place-items-center bg-slate-950/35 p-4 backdrop-blur-[2px]" onMouseDown={event => { if (event.currentTarget === event.target) onClose(); }}><form role="dialog" aria-modal="true" aria-label="创建新任务" onSubmit={(event: FormEvent) => { event.preventDefault(); create.mutate(); }} className="ui-dialog max-h-[92dvh] w-full max-w-md overflow-y-auto rounded-2xl border border-slate-200 bg-[var(--paper)] p-5 shadow-[0_24px_80px_rgba(15,23,42,.22)]"><div className="flex items-center"><div><h2 className="text-base font-semibold">创建新任务</h2><p className="mt-1 text-xs text-zinc-500">写清目标即可开始；模型和推理强度可以使用 Agent 默认值。</p></div><span className="ml-auto"><IconButton label="关闭" onClick={onClose}><X size={16}/></IconButton></span></div>
    <div className="mt-5 space-y-4"><label className="block"><span className="text-xs font-semibold text-slate-800">任务目标</span><textarea autoFocus required aria-label="任务目标" value={goal} onChange={event => setGoal(event.target.value)} rows={3} placeholder="例如：修复登录超时问题，补齐回归测试并通过构建" className="mt-1.5 w-full resize-y rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm leading-6 outline-none placeholder:text-slate-400 focus:border-teal-600"/></label><fieldset disabled={Boolean(createdSession)} className="contents disabled:opacity-60"><div><span className="text-xs font-medium text-zinc-700">执行任务的 Agent</span><AgentSelect agents={agents} value={agentId} onChange={value => { const nextAgent = agents.find(agent => agent.id === value); setAgentId(value); setPermissionMode(initialPermissionMode(nextAgent)); setFullTrustConfirmed(false); setModel(''); setReasoningEffort(''); }}/></div><label className="block"><span className="text-xs font-medium text-zinc-700">工作目录</span><span className="mt-1.5 flex gap-2"><input value={cwd} onChange={event => setCwd(event.target.value)} placeholder="留空则使用 Agent 默认目录" className="h-10 min-w-0 flex-1 rounded-lg border border-zinc-300 px-3 text-sm outline-none placeholder:text-zinc-400 focus:border-zinc-500"/><button type="button" disabled={!capabilities?.directoryPicker || pickNewWorkspace.isPending} onClick={() => pickNewWorkspace.mutate()} className="flex h-10 shrink-0 items-center gap-1.5 rounded-lg border border-zinc-300 bg-white px-3 text-xs font-medium text-zinc-700 hover:bg-zinc-50 disabled:cursor-not-allowed disabled:opacity-40"><FolderOpen size={14}/>{capabilities?.directoryPicker ? '选择目录' : '手动输入'}</button></span><p className="mt-1.5 text-[11px] leading-4 text-zinc-500">填写运行 Dockmux 的这台机器上的目录；留空会使用 Agent 的默认工作区。</p>{pickNewWorkspace.error && <p className="mt-1.5 text-xs text-red-600">{pickNewWorkspace.error.message}</p>}</label><div><span className="text-xs font-medium text-zinc-700">操作权限</span><CompactSelect options={supportedPermissionModes(selectedAgent).map(value => ({ value, ...permissionOptions[value] }))} value={permissionMode} placeholder="选择操作权限" disabledText="" onChange={value => { setPermissionMode(value as PermissionMode); setFullTrustConfirmed(false); }}/>{isPtyAgent(selectedAgent) && permissionMode === 'ask' && <p className="mt-1.5 text-[11px] leading-4 text-amber-700">此 CLI 的操作确认在终端中完成；运行停住时可直接打开「终端」处理。</p>}{permissionMode === 'full-trust' && <label className="mt-2 flex items-start gap-2 rounded-lg border border-rose-200 bg-rose-50 p-2.5 text-[11px] leading-4 text-rose-800"><input type="checkbox" checked={fullTrustConfirmed} onChange={event => setFullTrustConfirmed(event.target.checked)} className="mt-0.5"/><span><strong className="block">确认允许 Agent 直接操作此工作目录</strong>完全信任会跳过操作确认，只用于你信任的任务和目录。</span></label>}</div><details className="rounded-xl border border-slate-200 bg-slate-50 p-3"><summary className="cursor-pointer text-xs font-semibold text-slate-700">模型与推理设置（可选）</summary><div className="mt-3 space-y-4"><div><span className="text-xs font-medium text-zinc-700">模型</span>{!agentModels.data && (agentModels.isLoading || agentModels.isFetching) ? <div className="mt-1.5 flex h-10 items-center rounded-lg border border-zinc-200 bg-white px-3 text-sm text-zinc-400"><RefreshCw size={13} className="mr-2 animate-spin"/>正在读取可用模型</div> : <CompactSelect options={[{ value: '', label: agentModels.data?.defaultModel ? `使用 Agent 默认模型 (${agentModels.data.defaultModel})` : '使用 Agent 默认模型' }, ...(agentModels.data?.models ?? []).map(item => ({ value: item.id, label: item.name, meta: item.name === item.id ? undefined : item.id }))]} value={model} placeholder="选择模型" disabledText="使用 Agent 默认模型" onChange={value => { setModel(value); setReasoningEffort(''); }}/>}</div>{agentModels.data?.source === 'acp' && agentModels.data.reasoningEfforts.length > 0 && <div><span className="text-xs font-medium text-zinc-700">推理强度</span><CompactSelect options={[{ value: '', label: agentModels.data.defaultReasoningEffort ? `使用模型默认强度 (${agentModels.data.defaultReasoningEffort})` : '使用模型默认强度' }, ...agentModels.data.reasoningEfforts.map(item => ({ value: item.id, label: item.name }))]} value={reasoningEffort} placeholder="选择推理强度" disabledText="" onChange={setReasoningEffort}/></div>}</div></details></fieldset>
    </div>
    {create.error && <p role="alert" className="mt-3 text-xs text-red-600">{createdSession ? `任务已创建，但目标发送失败：${create.error.message}。再次提交只会重试发送，不会重复创建任务。` : create.error.message}</p>}<div className="mt-6 flex justify-end gap-2"><button type="button" onClick={onClose} className="min-h-10 rounded-lg border border-zinc-300 px-3.5 text-xs font-medium text-zinc-700 hover:bg-zinc-50">取消</button><button type="submit" disabled={submitDisabled} className="min-h-10 min-w-24 rounded-lg bg-teal-700 px-3.5 text-xs font-semibold text-white hover:bg-teal-600 disabled:opacity-50">{create.isPending ? <RefreshCw className="mx-auto animate-spin" size={14}/> : createdSession ? '重试发送' : '创建并执行'}</button></div>
  </form></div>;
}

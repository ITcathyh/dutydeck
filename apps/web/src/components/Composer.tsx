...
function QueuedTasks({ tasks, cancellingTaskId, steeringTaskId, onCancel, onSteer }: { tasks: Task[]; cancellingTaskId?: string; steeringTaskId?: string; onCancel(id: string): void; onSteer(id: string): void }) {
  ...
  return <div className="mx-4 overflow-hidden rounded-t-2xl border border-b-0 border-indigo-200/80 bg-indigo-50/90 shadow-[0_-5px_18px_rgba(30,41,59,.035)] backdrop-blur">
    <div className="flex items-center gap-2 border-b border-indigo-100 px-3.5 py-2 text-[11px] font-semibold text-indigo-700"><ListEnd size={13}/><span>待执行指令</span><span className="rounded-full bg-indigo-100 px-1.5 py-0.5 text-[10px] tabular-nums">{tasks.length}</span><span className="ml-auto text-[9px] font-normal text-indigo-500">当前任务完成后依次执行</span></div>
    <div className="max-h-28 overflow-y-auto">{tasks.map(task => <div key={task.id} className="group flex items-center gap-2.5 px-3.5 py-2 text-[12px] text-zinc-600 hover:bg-zinc-50"><span className="min-w-0 flex-1 truncate">{task.prompt}</span><span className="text-[10px] text-zinc-400">排队中</span><button type="button" disabled={Boolean(cancellingTaskId) || Boolean(steeringTaskId)} onClick={() => onSteer(task.id)} className="h-6 rounded-md px-2 text-[10px] font-medium hover:bg-orange-600 hover:text-white disabled:opacity-35">{steeringTaskId === task.id ? '正在打断' : '打断当前任务并执行'}</button><button type="button" disabled={Boolean(cancellingTaskId) || Boolean(steeringTaskId)} onClick={() => onCancel(task.id)} aria-label={`取消排队：${task.prompt}`} className="grid h-6 w-6 place-items-center rounded-md text-zinc-400 hover:bg-zinc-200 hover:text-zinc-700 disabled:opacity-30"><X size={12}/></button></div>)}</div>
  </div>;
}
...
export function Composer({ state, value, references, sending, mode, queuedTasks, cancellingTaskId, steeringTaskId, skills, models, reasoningEfforts, currentModel, currentReasoningEffort, context, advertisedCommands, filePicker, modelReadiness, switchingModel, switchingReasoningEffort, refreshingModels, onChange, onReferencesChange, onModeChange, onSubmit, onInterrupt, onCancelQueued, onSteerQueued, onPickFile, onModelChange, onReasoningEffortChange, onRefreshModels }: {
  ...
  return <div className="bg-gradient-to-t from-[var(--canvas)] via-[var(--canvas)] to-transparent px-4 pb-5 pt-7 sm:px-8"><div className="mx-auto max-w-[880px]">
    ...
    <div className={`relative rounded-2xl border border-slate-200 bg-[var(--paper)] p-2 shadow-[0_1px_2px_rgba(15,23,42,.04),0_12px_34px_rgba(15,23,42,.07)] ring-1 ring-inset ring-white/80 backdrop-blur-sm transition-[transform,border-color,box-shadow,background-color] duration-300 ease-out focus-within:-translate-y-0.5 focus-within:border-teal-500/45 focus-within:bg-white focus-within:shadow-[0_2px_4px_rgba(15,23,42,.04),0_16px_42px_rgba(15,23,42,.1)] ${queuedTasks.length ? 'rounded-t-[10px]' : ''}`}>
      ...
      <div className="flex items-center px-2.5 pt-1 text-[9px] font-semibold uppercase tracking-[.13em] text-teal-700">{busy ? '追加执行要求' : '下一步指令'}<span className="ml-auto font-normal normal-case tracking-normal text-slate-400">Enter 执行 · Shift + Enter 换行</span></div><textarea ref={textarea} aria-label="消息" value={value} onChange={event => { onChange(event.target.value); if (panel !== 'file' && panel !== 'models' && panel !== 'reasoning') setPanel(undefined); }} onKeyDown={keyboard} placeholder={busy ? '补充要求；可选择排队或立即介入当前执行' : '描述要完成的目标、范围和验收要求…'} rows={1} className="max-h-48 min-h-16 w-full resize-none overflow-y-auto border-0 bg-transparent px-2.5 py-2 text-[14px] leading-6 text-slate-900 outline-none placeholder:text-slate-400 focus-visible:outline-none"/>

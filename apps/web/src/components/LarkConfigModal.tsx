      ...
      <Dialog.Body className="space-y-3.5">
        {config.isLoading ? <Skeleton variant="row" lines={3}/> : <>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field label="App ID"><Input value={appId} onChange={event => { setAppId(event.target.value); setName(''); setOpenPlatformJobId(''); }} placeholder="cli_xxx" autoComplete="off" className="font-mono"/></Field>
          </div>
          {/*
            🔒 凭据只写不读。

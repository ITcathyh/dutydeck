...
export function LarkConfigModal({ agents, target, onClose }: LarkConfigModalProps) {
  ...
  const installHook = useMutation({
    mutationFn: async () => {
      const saved = await api.saveLarkConfig({ stage: 'agent', originalAppId: current!.appId, defaultAgentId, defaultModel, defaultReasoningEffort, fullTrustConfirmed, permissionMode, preInjectPrompt, listening, groupToolsEnabled, groupToolsAllowSend, highRiskAllowedUserNames, highRiskAllowedEmails: [], highRiskPattern, riskControlMode: 'guidance' });

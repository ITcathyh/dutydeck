...
export function LarkConfigModal({ agents, target, onClose }: LarkConfigModalProps) {
  ...
  useEffect(() => {
    ...
    setName(bot?.name ?? ''); setWorkspace(bot?.workspace ?? ''); setWebBaseUrl(bot?.webBaseUrl ?? config.data.bots.find(item => item.webBaseUrl)?.webBaseUrl ?? (typeof window !== 'undefined' ? window.location.origin : '')); setWebMobileReachable(bot?.webMobileReachable !== false); setAppId(bot?.appId ?? ''); setAppSecret(''); setOpenPlatformJobId(''); setRestrictUsers(Boolean(bot?.allowedUsers.length || bot?.allowedEmails.length)); setAllowedUserNames((bot?.allowedUsers ?? []).map(user => user.name)); setAllowedBotNames((bot?.allowedBots ?? []).map(b => b.name)); setPeerBotsAllowed(bot?.peerBotsAllowed !== false);
    setDefaultAgentId(agents.some(agent => agent.id === bot?.defaultAgentId) ? bot?.defaultAgentId ?? '' : agents[0]?.id ?? '');
    setDefaultModel(bot?.defaultModel ?? ''); setDefaultReasoningEffort(bot?.defaultReasoningEffort ?? ''); setFullTrustConfirmed(bot?.fullTrustConfirmed ?? false); setPermissionMode(bot?.permissionMode ?? 'full-trust'); setListening(enableListeningAfterNewBot.current ? true : bot?.listening ?? true); enableListeningAfterNewBot.current = false; setPushIntervalMs(bot?.pushIntervalMs ?? 1000); setTraceLimit((bot?.traceLimit ?? 10).toString()); setCompactTrace(bot?.compactTrace ?? true);
    setPreInjectPrompt(bot?.preInjectPrompt ?? ''); setGroupToolsEnabled(bot?.groupToolsEnabled ?? false); setGroupToolsAllowSend(bot?.groupToolsAllowSend ?? false); setMemoryEnabled(bot?.memoryEnabled ?? (bot?.defaultGroupParticipation === 'selective')); setMemoryAutoExtract(bot?.memoryAutoExtract ?? true); setMemoryAgentId(bot?.memoryAgentId ?? ''); setMemoryModel(bot?.memoryModel ?? ''); setStructuredAskCards(bot?.structuredAskCards ?? true); setGroupCardMention(bot?.groupCardMention ?? true); setCompletionReactionOnly(bot?.completionReactionOnly ?? false); setSilentProgress(bot?.silentProgress ?? false); setWorkspaceAliasRows(Object.entries(bot?.workspaceAliases ?? {}).map(([alias, path]) => ({ alias, path }))); setVerificationCommand(bot?.verificationCommand ?? '');
  ...
  const save = useMutation({
    mutationFn: () => step === 1
      ? api.saveLarkConfig({ stage: 'lark', ...(current ? { originalAppId: current.appId } : {}), appId: appId.trim(), ...(appSecret.trim() ? { appSecret: appSecret.trim() } : {}), workspace: workspace.trim(), workspaceAliases: workspaceAliasTable(workspaceAliasRows), verificationCommand: verificationCommand.trim(), webBaseUrl: webBaseUrl.trim(), webMobileReachable, structuredAskCards, groupCardMention, completionReactionOnly, silentProgress, urgentEnabled, urgentThresholdMs: urgentThresholdMs ?? null, urgentMaxPerHourPerChat: urgentQuota ?? null, pinLongTasks, pinAfterMs: pinAfterMs ?? null, allowedUserNames: restrictUsers ? allowedUserNames : [], allowedEmails: [], allowedBotNames, peerBotsAllowed, pushIntervalMs, traceLimit: Number(traceLimit), compactTrace })
  ...
  const canSave = !creationBusy && !missingTarget && (step === 1
    ...
    : Boolean(current && defaultAgentId && (permissionMode === 'full-trust' ? fullTrustConfirmed : agentOptions.data?.source === 'acp') && agentCapabilitiesReady && (riskControlMode === 'off' || highRiskPatternValidation.valid) && !legacyHighRiskNeedsMigration && (riskControlMode !== 'enforced' || enforcedHookReady)));
      ...
      <Dialog.Body className="space-y-3.5">
        {config.isLoading ? <Skeleton variant="row" lines={3}/> : missingTarget ? <Banner tone="warning">找不到指定机器人。请选择已有机器人或点击新增机器人。</Banner> : <>
          ...
          */}
          ...
          <div className="flex items-center rounded-md bg-surface px-3 py-2 shadow-card"><div className="min-w-0"><label htmlFor="lark-switch-web-mobile" className="cursor-pointer text-caption font-medium text-primary">手机能打开这个地址</label><div className="mt-0.5 text-caption text-subtle">卡片上的“查看详情”在手机飞书里也会打开这个地址。手机打不开就关掉：卡片不再显示“查看详情”，地址也不保存。</div></div><Switch id="lark-switch-web-mobile" label="手机能打开这个地址" checked={webMobileReachable} onToggle={() => setWebMobileReachable(value => !value)}/></div>
          {/* S7：留空或只绑本机/内网时，卡片的“查看详情”在手机上注定打不开，必须就地提示，不做公网兜底。 */}
          {webMobileReachable && webBaseUrlReachability.kind !== 'public' && <Banner tone="warning">{webBaseUrlReachability.message}</Banner>}
          ...
          <div className="space-y-2.5 border-t border-subtle pt-3.5">
            ...
            <div className="flex items-center rounded-md bg-surface px-3 py-2 shadow-card"><div className="min-w-0"><label htmlFor="lark-switch-compact-trace" className="cursor-pointer text-caption font-medium text-primary">精简过程卡（默认开启）</label><div className="mt-0.5 text-caption text-subtle">执行过程只展示各阶段标题与 Agent 旁白，不展开每条工具调用；关闭后恢复完整执行记录。完整记录可在 Web 查看。</div></div><Switch id="lark-switch-compact-trace" label="精简过程卡（默认开启）" checked={compactTrace} onToggle={() => setCompactTrace(value => !value)}/></div>
            <div className="flex items-center rounded-md bg-surface px-3 py-2 shadow-card"><div className="min-w-0"><label htmlFor="lark-switch-group-mention" className="cursor-pointer text-caption font-medium text-primary">群卡片 @ 发起人（默认开启）</label><div className="mt-0.5 text-caption text-subtle">开启后群内结果卡末尾 @ 任务发起人，审批卡与提问卡在卡首 @；私聊不受影响。可在群配置里按群覆盖。</div></div><Switch id="lark-switch-group-mention" label="群卡片 @ 发起人（默认开启）" checked={groupCardMention} onToggle={() => setGroupCardMention(value => !value)}/></div>
          ...
          <Field label="操作确认方式" hint="飞书逐项确认与只读自动放行支持 ACP Agent；修改后在会话空闲时应用，新操作仍受群权限与高危策略约束。"><CompactSelect options={[{ value: 'full-trust', label: '完全信任：自动执行操作' }, { value: 'approve-reads', label: '只读自动放行：读取与搜索直接执行，其余逐项确认' }, { value: 'ask', label: '飞书逐项确认：等待批准后执行' }]} value={permissionMode} placeholder="选择操作确认方式" disabledText="" onChange={value => setPermissionMode(value as 'ask' | 'approve-reads' | 'full-trust')}/></Field>
          {permissionMode !== 'full-trust' && agentOptions.data?.source !== 'acp' && <p className="text-caption text-warning">当前 Agent 尚未确认支持 ACP，请选择支持 ACP 的 Agent 后保存。</p>}

...
export default function App() {
  ...
  const renderNotFound = (kind: 'page' | 'session') => <div className="relative grid min-h-0 flex-1 place-items-center overflow-auto p-6"><div className="absolute left-3 top-3 md:hidden"><IconButton label="打开工作台导航" onClick={openMobileNavigation}><Menu size={17}/></IconButton></div><section className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-7 text-center shadow-sm"><p className="text-[10px] font-semibold uppercase tracking-[.12em] text-amber-700">{kind === 'page' ? 'Page not found' : 'Session not found'}</p><h1 className="mt-2 text-lg font-semibold text-slate-900">{kind === 'page' ? '找不到这个页面' : '找不到这个任务运行'}</h1><p className="mt-2 text-xs leading-5 text-slate-500">{kind === 'page' ? '当前链接不是有效的 Dockmux 页面。' : '它可能已被删除，或当前链接不属于这个 Dockmux 实例。'}</p><button type="button" onClick={() => selectSession(undefined)} className="mt-5 rounded-lg bg-slate-900 px-3.5 py-2 text-xs font-semibold text-white hover:bg-slate-700">回到任务中心</button></section></div>;
  ...
  return <div className="relative flex h-[100dvh] min-h-[100dvh] overflow-hidden bg-[var(--canvas)] font-sans text-slate-900">
    ...
    <main aria-hidden={mobileNavigationOpen || undefined} inert={mobileNavigationOpen || undefined} className="flex min-w-0 flex-1 flex-col">
      {route.kind !== 'not-found' && !blockingMainQueryFailures.length && staleMainQueryFailures.length > 0 && <div role="status" className="flex shrink-0 items-center gap-3 border-b border-amber-200 bg-amber-50 px-4 py-2 text-xs text-amber-900"><span className="min-w-0 flex-1 truncate" title={staleMainQueryFailures.map(failure => `${failure.label}：${failure.message}`).join('\n')}>部分数据可能不是最新：{staleMainQueryFailures.map(failure => failure.label).join('、')}</span><button type="button" disabled={retryingMainQueries} onClick={() => void Promise.all(staleMainQueryFailures.map(failure => failure.retry()))} className="shrink-0 rounded-md border border-amber-300 bg-white px-2 py-1 font-semibold hover:bg-amber-100 disabled:opacity-50">{retryingMainQueries ? '重试中…' : '重试'}</button></div>}
      {route.kind === 'not-found' ? renderNotFound('page') : blockingMainQueryFailures.length ? <div className="relative grid min-h-0 flex-1 place-items-center overflow-auto p-6"><div className="absolute left-3 top-3 md:hidden"><IconButton label="打开工作台导航" onClick={openMobileNavigation}><Menu size={17}/></IconButton></div><section role="alert" className="w-full max-w-lg rounded-2xl border border-rose-200 bg-white p-6 shadow-sm"><p className="text-[10px] font-semibold uppercase tracking-[.12em] text-rose-600">数据未就绪</p><h1 className="mt-2 text-lg font-semibold text-slate-900">无法加载任务中心</h1><p className="mt-1 text-xs leading-5 text-slate-500">以下关键数据加载失败。为避免把故障显示成空列表，任务内容已暂停展示。</p><ul className="mt-4 space-y-2">{mainQueryFailures.map(failure => <li key={failure.label} className="rounded-lg bg-rose-50 px-3 py-2 text-xs text-rose-800"><strong>{failure.label}</strong><span className="ml-2 text-rose-600">{failure.message}</span></li>)}</ul><button type="button" disabled={retryingMainQueries} onClick={() => void Promise.all(mainQueryFailures.map(failure => failure.retry()))} className="mt-5 rounded-lg bg-slate-900 px-3.5 py-2 text-xs font-semibold text-white hover:bg-slate-700 disabled:opacity-50">{retryingMainQueries ? '正在重试…' : '重新加载'}</button></section></div> : route.kind === 'session' && sessions.isPending ? <div className="grid min-h-0 flex-1 place-items-center text-xs text-slate-500">正在加载任务运行…</div> : missingActiveSession ? renderNotFound('session') : active ? <>
    ...
    {larkSetupOpen && <Suspense fallback={<div className="fixed inset-0 z-30 grid place-items-center bg-slate-950/30 text-sm text-white backdrop-blur-sm">正在打开 Bot 绑定向导…</div>}><LarkConfigModal agents={agents.data ?? []} onClose={() => { setLarkSetupOpen(false); void qc.invalidateQueries({ queryKey: ['lark-config'] }); }}/></Suspense>}
    {controlOpen && <Suspense fallback={<div className="fixed inset-0 z-30 grid place-items-center bg-slate-950/30 text-sm text-white backdrop-blur-sm">正在打开设置与接入…</div>}><ControlCenterModal open initialSection={controlSection} agents={agents.data ?? []} legacyBots={larkConfig.data?.bots ?? []} authRequired={authStatus.data?.required} onClose={() => setControlOpen(false)} onCreateTask={() => { setControlOpen(false); openCreateTask(); }} onOpenLarkSetup={openLarkSetup} onOpenGroups={() => { setControlOpen(false); setGroupPolicyOpen(true); }} onOpenSchedules={() => { setControlOpen(false); setScheduleOpen(true); }}/></Suspense>}
    {groupPolicyOpen && <Suspense fallback={<div className="fixed inset-0 z-30 grid place-items-center bg-slate-950/30 text-sm text-white backdrop-blur-sm">正在打开群配置…</div>}><GroupPolicyModal open onClose={() => setGroupPolicyOpen(false)}/></Suspense>}
    {scheduleOpen && <Suspense fallback={<div className="fixed inset-0 z-30 grid place-items-center bg-slate-950/30 text-sm text-white backdrop-blur-sm">正在打开自动化…</div>}><ScheduleFoundationPanel open onClose={() => setScheduleOpen(false)}/></Suspense>}

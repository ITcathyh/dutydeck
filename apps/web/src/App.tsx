...
import { Menu, PanelRightClose, Terminal } from 'lucide-react';
...
import { busyStates, effectiveStatus } from './components/ui';
import { Banner, Button, Card, IconButton, Spinner } from './components/primitives';
...
export default function App() {
  ...
  const openLarkSetup = () => openOverlay({ kind: 'lark-setup' });
  ...
  const visibleSessions = useMemo(() => sortedSessions.filter(session => session.source !== 'work_item'), [sortedSessions]);
  ...
  const overlayOpen = Boolean(overlay) || newOpen || archiveConfirm || systemPromptOpen || paletteOpen || helpOpen || deliveryPanelOpen;
    ...
    */}
    <div className="relative flex min-h-0 flex-1 overflow-hidden">
      ...
      */}
      ...
      </> : <WorkspaceOverview sessions={visibleSessions} summaries={runSummaries} agents={agents.data ?? []} loading={sessions.isLoading} agentsLoading={agents.isLoading} larkBots={larkConfig.data?.bots ?? []} larkBotsLoading={larkConfig.isLoading} larkListeningDisabled={larkConfig.data?.listeningDisabled ?? false} larkBotsFailed={larkConfig.isError} larkBotsRetrying={larkConfig.isFetching} onRetryLarkBots={() => void larkConfig.refetch()} view={workbenchView} onViewChange={setWorkbenchView} onSelect={selectSession} onCreate={openCreateTask} onOpenAgentSetup={() => openSettings('agents')} onOpenLarkSetup={openLarkSetup}/>}
    </main>
    </div>

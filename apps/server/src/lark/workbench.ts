...
import { sendLarkResult } from './result-delivery.js';
...
export function workItemElements(item: WorkItem, requests: WorkItemRequest[] = []) {
  ...
  for (const step of item.steps) {
    ...
    elements.push({ tag: 'markdown', content: `**${labels[step.status]} · ${definition.title}**${definition.agentId ? ` · ${definition.agentId}` : ''}\n${attempt?.error?.slice(0, 1200) ?? ''}${step.status === 'waiting' ? `\n${definition.instruction}\n\n回复：\`/work answer ${item.id} ${step.id} 你的回答\`` : ''}` });
  ...
  for (const request of requests) {
    ...
    else elements.push({ tag: 'markdown', content: `回复：\`/work respond ${item.id} ${request.stepId} ${request.requestId} 你的回答\`` });
  }
...
export class LarkWorkbench {
  ...
  private async send(item: WorkItem, target: Target, config: StoredLarkConfig, key: string, requests: WorkItemRequest[] = []) {
    const result = await sendLarkResult(this.client(config), target, {
      state: item.status === 'completed' ? 'completed' : ['failed', 'blocked'].includes(item.status) ? 'failed' : item.status === 'cancelled' ? 'interrupted' : 'running',
      readOnly: true, retryable: false, taskId: item.id, taskName: item.title, sessionId: item.parentSessionId,
      webBaseUrl: config.webBaseUrl, elements: workItemElements(item, requests), idempotencyKey: key
    }, this.options.log);
    this.assertOpen();
    await this.repos.channelMappings.save({ id: `workcard_${digest(`${config.appId}\0${result.messageId}`)}`, channel: `lark-work-card:${config.appId}`, externalId: result.messageId,
      sessionId: item.parentSessionId, createdAt: new Date().toISOString(), extra: JSON.stringify({ workId: item.id, chatId: target.chatId, revision: item.revision }) });
  }
  ...
  async notify(item: WorkItem, actorId: string) {
    ...
    const config = await readLarkConfig(this.repos.config, target.appId);
    if (!config) return;
    const fingerprint = digest(JSON.stringify([item.status, item.steps.map(step => [step.id, step.status, step.attempts.at(-1)?.error]), requests]));
    const key = `workbench.notice.${item.id}.${fingerprint}`;
    if (await this.repos.config.get(key)) return;
    await this.send(item, target, config, `work_${digest(key).slice(0, 40)}`, requests);
    this.assertOpen();
  ...
  async callback(value: Record<string, unknown>, actorId: string | undefined, context: { messageId?: string; chatId?: string }, config: StoredLarkConfig) {
    ...
    if (!mapping || !saved || saved.chatId !== context.chatId || saved.workId !== value.work_id || mapping.sessionId !== value.parent_session_id || saved.revision !== value.revision) throw new RuntimeError('WORK_ITEM_CALLBACK_INVALID', '卡片已过期或不属于此话题', 409);
    ...
    if (operation === 'cancel') await this.work().cancel(mapping.sessionId, item.id, Number(value.revision), actorId);
    else if (operation === 'retry') await this.work().retryStep(mapping.sessionId, item.id, String(value.step_id), Number(value.revision), actorId);
    else if (operation === 'respond') await this.interactions().respond(mapping.sessionId, item.id, { stepId: value.step_id, taskId: value.task_id, requestId: value.request_id, kind: value.request_kind, answer: value.answer }, actorId);
    else if (operation !== 'show') throw new RuntimeError('WORK_ITEM_CALLBACK_INVALID', '无法识别目标操作', 400);
    const updated = await this.work().get(mapping.sessionId, item.id, actorId);
    // Reply to the card that was actually clicked; all actions retain the original parent scope.
    await this.send(updated, { appId: config.appId, chatId: context.chatId, replyMessageId: context.messageId, replyInThread: true }, config,
      `work_${digest(`${context.messageId}\0${updated.revision}\0${JSON.stringify(value)}`).slice(0, 40)}`, await this.interactions().list(mapping.sessionId, item.id, actorId));
    return '目标状态已更新';
  }
}

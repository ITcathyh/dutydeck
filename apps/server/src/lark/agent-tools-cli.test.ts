...
describe('Agent group tool CLI client', () => {
  it('requires the scoped Dockmux Lark session capability', () => {
  ...
  it('discovers human members through the same scoped capability', async () => {
    ...
    await runGroupMembers({
      env: { dockmux_group_tools_url: 'http://127.0.0.1:4310/api/lark/agent-tools', dockmux_group_tools_token: 'capability-token' },
      fetcher: fetcher as typeof fetch
    });
  ...
  it('preserves actionable authorization details from the broker', async () => {
    ...
    await expect(runGroupMessages({}, {
      env: { DOCKMUX_GROUP_TOOLS_URL: 'http://127.0.0.1:4310/api/lark/agent-tools', DOCKMUX_GROUP_TOOLS_TOKEN: 'token' },
      fetcher: fetcher as typeof fetch

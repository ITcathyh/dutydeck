...
// canonical `migrate` 承载完整的加解密与安全黑盒测试；兼容别名 `botmux` 仅保留轻量分发验证，避免重复拉起数十次黑盒进程。
describe('dutydeck migrate CLI black box', () => {
  const group = 'migrate';
...
describe('dutydeck botmux CLI alias', () => {
  it('exposes only the three read-only migration subcommands through the botmux alias', async () => {
    const source = await fixture();

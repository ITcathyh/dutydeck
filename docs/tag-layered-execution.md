...
6. 每个子会话在结果保存后由后台停止（完成或失败都停，不占调度循环；普通单步重试起新会话）。验收采用持久化结构化 review（`decision`: `accept` / `rework` / `stop`）：默认最多自动返修 2 轮（自定义编排支持 0..3 轮）；返修时复用原 worktree 工作区重新运行被指定的末端 Worker，完成后再次由 Leader 验收。仅在验收判定为 `accept` 时目标整体标记完成；若判定为 `stop`、返修次数超限或审查结论不合法，目标进入阻塞状态（`blocked`），不继续下一轮；用户主动取消则立即停止后续轮次。卡片跟随结构化审查状态；历史无 reviewPolicy 的目标仍兼容原‘验收结论’文本规则。
...
- 审查与返修边界：reviewPolicy 仅允许设置在最终输出步骤，且返修目标必须是只被最终验收依赖的无条件末端步骤，并指派给与验收 Leader 不同的 Agent（保证独立验收）；返修时直接复用原 Worker 的工作目录与 worktree。

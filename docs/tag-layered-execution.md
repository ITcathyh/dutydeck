...
6. 每个子会话在结果保存后由后台停止（完成或失败都停，不占调度循环；重试会起新会话，不复用旧会话）。验收步骤的最终答复第一行写「验收结论：通过 / 需返修 / 缺少信息」，作为结果卡回到原话题；终端模式 Leader 调工具前说的话也会出现在产物里，所以卡片按最后一个以「验收结论：」开头的行判断，结论不是「通过」或无法识别时，卡片显示「验收需返修」「验收缺少信息」或「验收待核对」，不显示「已完成」。

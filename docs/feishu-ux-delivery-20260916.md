...
1. 排队/取消/中断的 actor 与真实终态
   - 排队取消、`/cancel`、卡片取消统一携带操作人 openId；按当前权限校验操作人；有排队时拒绝匿名stop，跨App请求在发送进程信号前拒绝，且在信号进程前完成全部身份校验，不会先停进程再报错。
...
7. 公开执行记录导出
   - 终态/恢复卡在有真实持久化任务与轮次时渲染“导出执行记录”；尚未执行的排队/取消任务不提供导出记录入口，旧卡点击给出暂无记录提示；回调先做人/群/轮次/`task.view_result` 授权，再按用户 prompt 与终态事件切分本轮边界，后台把脱敏的“公开执行记录.md”发回原会话。旧快照里的“去 Web 查看”死文案被清理。

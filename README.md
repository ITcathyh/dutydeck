...
- **任务优先的 Web 工作台**：侧栏按工作目录聚合任务，首页按待你处理、进行中和已完成分组。创建任务时先写目标，再按需覆盖 Agent、模型和推理强度。
- **飞书指挥台**：私聊或群聊 @ 即可下达任务。固定确认表情承诺已接收，可变进度卡展示排队与执行，终态冻结为收据并另发结果通知；IM 用于通知与决策，完整证据在 Web 深挖。
...
- Dockmux 在消息接入后立即用固定 `OK` 确认收到；随后在触发消息或对应话题下回复可变进度卡。解析失败也会给出可见失败回执，不会静默。
- 排队、运行、待决策、完成、中断和失败使用不同视觉层级。
- 运行态只保留少量合并后的有效进展；终态将原进度卡冻结为简洁收据，并新发一条结论优先的结果/失败通知。这样即使飞书 PATCH 历史卡片不产生新提醒，用户仍能收到完成通知。
- 默认完成通知隐藏详细 trace，只保留简短证据摘要并提供 Web 详情入口；显式关闭隐藏时，详细轨迹仍以折叠区提供。
- 群聊继续任务需回复并 @机器人；私聊可直接继续发送。重试会创建新的进度卡，不改写已冻结的终态收据。
- 卡片不展示模型私有思维链。工具参数、输出、错误和终端摘要在进入卡片前会做敏感信息清理并受卡片总大小预算约束。
- daemon 重启后，恢复中的旧卡片先变为只读，再与持久化终态对账。结果通知 ID、交付状态和幂等键会持久化并在恢复时复用，避免制造重复结果通知。

...
async function runTest() {
  ...
  await waitFor('all turns completed before restart', async () => {
    const sessions = await harness!.runtime.listSessions();
    if (sessions.length === 0) return false;
    let totalCompletedTasks = 0;
    for (const session of sessions) {
      if (session.state !== 'idle') return false;
      const tasks = await harness!.repositories.tasks.listBySession(session.id);
      if (tasks.length === 0 || tasks.some(t => t.status !== 'completed')) return false;
      totalCompletedTasks += tasks.length;
    }
    return totalCompletedTasks === 6;
  });
  ...
  const resumedSession = (await harness.runtime.listSessions()).find(s => s.sourceId?.includes(first.messageId) && s.sourceId.startsWith('cli_one:'))!;
  assert(resumedSession, 'Missing resumed session after restart');
  const preResumeTaskIds = new Set((await harness.repositories.tasks.listBySession(resumedSession.id)).map(t => t.id));
  ...
  await waitFor('capture resume task id', async () => {
    const currentTasks = await harness!.repositories.tasks.listBySession(resumedSession.id);
    const newTask = currentTasks.find(t => !preResumeTaskIds.has(t.id));
    if (newTask) {
      resumeTaskId = newTask.id;
      return true;
    }
    return false;
  });
  assert(resumeTaskId, 'Resume task must be captured with unique ID');
  await waitFor('persisted session resume', async () => {
    const obs = await harness!.observations();
    if (obs.length < 7) return false;
    const session = await harness!.runtime.getSession(resumedSession.id);
    const currentTasks = await harness!.repositories.tasks.listBySession(resumedSession.id);
    const target = currentTasks.find(t => t.id === resumeTaskId);
    return session?.state === 'idle' && target?.status === 'completed';
  });
  assert.equal((await harness.observations()).length, 7);
  const finalResumeTasks = await harness.repositories.tasks.listBySession(resumedSession.id);
  const newResumeTaskIds = finalResumeTasks.filter(t => !preResumeTaskIds.has(t.id)).map(t => t.id);
  assert.deepEqual(newResumeTaskIds, [resumeTaskId]);
  const finalResumeTask = finalResumeTasks.find(t => t.id === resumeTaskId);
  assert.equal(finalResumeTask?.status, 'completed');
  assert.equal(finalResumeTask?.id, resumeTaskId);
  ...
  const queuedBobTask = (await harness!.repositories.tasks.listBySession(heldSession.id)).find(task => task.status === 'queued' && task.executionContext?.actorId === 'ou_bob');
  assert(queuedBobTask, 'Missing queued Bob task');
  const bobTaskId = queuedBobTask.id;
  ...
  await waitFor('queued turn completed', async () => {
    const session = await harness!.runtime.getSession(heldSession.id);
    const tasks = await harness!.repositories.tasks.listBySession(heldSession.id);
    const target = tasks.find(t => t.id === bobTaskId);
    return session?.state === 'idle' && target?.status === 'completed';
  });
  const heldTasks = await harness.repositories.tasks.listBySession(heldSession.id);
  const finalBobTask = heldTasks.find(task => task.id === bobTaskId);
  assert(finalBobTask, 'Bob task must exist');
  assert.equal(finalBobTask.status, 'completed');
  assert.equal(finalBobTask.executionContext?.actorId, 'ou_bob');
  assert.equal((await harness.runtime.getSession(heldSession.id))?.state, 'idle');
  ...
  assert(oldSession);
  const ownerSendRes = await request('POST', `/api/sessions/${oldSession.id}/send`, { prompt: 'dashboard owner continuation' });
  assert.ok(ownerSendRes?.task?.id, 'ownerSendRes must return task.id');
  const ownerTaskId: string = ownerSendRes.task.id;
  await waitFor('Dashboard owner continuation', async () => {
    if ((await harness!.observations()).length < 10) return false;
    const session = await harness!.runtime.getSession(oldSession.id);
    const tasks = await harness!.repositories.tasks.listBySession(oldSession.id);
    const target = tasks.find(t => t.id === ownerTaskId);
    return session?.state === 'idle' && target?.status === 'completed';
  });
  assert.equal((await harness.observations()).length, 10);
  const ownerTasks = await harness.repositories.tasks.listBySession(oldSession.id);
  const ownerTask = ownerTasks.find(task => task.id === ownerTaskId && task.executionContext?.actorId === 'installation_owner');
  assert(ownerTask, 'Missing owner task with installation_owner actor matching fixed ownerTaskId');
  assert.equal(ownerTask.status, 'completed');

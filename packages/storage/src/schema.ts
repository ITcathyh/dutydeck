...
export const sessions = sqliteTable('sessions', { id: text('id').primaryKey(), agentId: text('agent_id').notNull(), state: text('state').notNull(), cwd: text('cwd').notNull(), model: text('model'), reasoningEffort: text('reasoning_effort'), systemPrompt: text('system_prompt'), permissionMode: text('permission_mode').default('full-trust'), source: text('source'), sourceId: text('source_id'), archivedAt: text('archived_at'), protocol: text('protocol'), runId: text('run_id').notNull(), error: text('error'), ...timestamps });
export const tasks = sqliteTable('tasks', { id: text('id').primaryKey(), sessionId: text('session_id').notNull(), prompt: text('prompt').notNull(), status: text('status').notNull(), ...timestamps });

 ...
 * records, and — when both come up empty — the dockmux session id itself.
 *
 * That last tier is only correct for CLIs where dockmux PINNED the id at fresh
 * spawn (`claude --session-id <uuid>`, `grok --session-id`, `pi`, `mtr`). For a
 * CLI that mints its own id, the dockmux id is a value the CLI has never seen,
 ...
 * mints `ses_<26 alnum>` in their OWN namespaces. What identifies a dockmux id
 ...
 * ---------------------------------------------------------
 * A bare UUID must NOT be treated as "dockmux's": codex and traex rollout ids
 ...
 * driver fell back to the dockmux id, so the only honest answer is `null`.
 *
 ...
 */
export function isDockmuxSessionId(sessionId: string): boolean {
  return DOCKMUX_SESSION_ID_RE.test(sessionId);
}
 ...
 *
 * Returns undefined when the id is absent or is dockmux's own session id (see
 * `isDockmuxSessionId`); an adapter whose CLI has a recognisable native id
...
export function usableResumeId(resumeSessionId: string | undefined): string | undefined {
  ...
  return isDockmuxSessionId(resumeSessionId) ? undefined : resumeSessionId;
}

 ...
 * 裸 `dutydeck` 可能命中另一个安装，其凭证在本会话无效）。静态常量拿不到运行期路径。
 *
 * 解法是**经会话 env 下发**：服务端注入子进程的 `dutydeck_relay_command` 就是那个
 * 运行期前缀，`relayHintLines(env)` 在拼文案时读回。所以下面的
 * `DUTYDECK_SHELL_HINTS` 退化为「与回传无关的通用提示」，回传相关的行由
 * `buildDutydeckRoutingBlock(locale, env)` 在运行期补。不传 env 时回退到
 * 「直接输出文本」的旧文案，行为不回归。
 */
import { relayHintLines, type RelayPromptEnv } from '@dutydeck/relay';

/** 与回传通道无关的通用提示（静态部分） */
export const DUTYDECK_SHELL_HINTS: readonly string[] = [
 ...
 */
export function buildDutydeckRoutingBlock(locale?: string, env?: RelayPromptEnv): string {
  void locale;
  return [
    '<dutydeck_routing>',
    ...DUTYDECK_SHELL_HINTS.map(hint => `  ${hint}`),
...
export function prependRoutingBlock(prompt: string, locale?: string, env?: RelayPromptEnv): string {
  return `${buildDutydeckRoutingBlock(locale, env)}\n\n${prompt}`;
}

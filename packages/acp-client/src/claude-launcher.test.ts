...
describe('Claude ACP launcher', () => {
  it('routes the built-in Claude agent through the Dockmux launcher', () => {
  ...
  it('loads Claude settings env without overriding explicit process variables and reuses the installed CLI', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dockmux-claude-launcher-')); dirs.push(root);
    ...
    const result = spawnSync(process.execPath, [resolve(process.cwd(), 'packages/acp-client/agents/claude-acp.mjs')], {
      cwd: project,
      env: { ...process.env, PATH: `${bin}${process.platform === 'win32' ? ';' : ':'}${process.env.PATH ?? ''}`, CLAUDE_CONFIG_DIR: configDir, ANTHROPIC_AUTH_TOKEN: 'explicit-token', DOCKMUX_CLAUDE_ACP_COMMAND: process.execPath, DOCKMUX_CLAUDE_ACP_ARGS_JSON: JSON.stringify(['-e', inspect]) },
      encoding: 'utf8'
    });
    expect(result.status).toBe(0);
    expect(JSON.parse(result.stdout)).toEqual({ token: 'explicit-token', base: 'https://project.example', model: 'local-model', bedrock: '1', awsProfile: 'dockmux-test', executable: claude });
  });
});

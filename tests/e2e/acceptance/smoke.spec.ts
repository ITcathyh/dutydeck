...
test.describe('Core smoke verification', () => {
  ...
  test('runs end-to-end smoke with isolated tmux and server', async ({}, testInfo) => {
    ...
    const smokeResultsPath = join(artifactDir, 'smoke-results.json');
    expect(existsSync(smokeResultsPath)).toBe(true);
    const smokeResults = JSON.parse(readFileSync(smokeResultsPath, 'utf8'));
    expect(smokeResults.passed).toBe(true);
    expect(Array.isArray(smokeResults.results)).toBe(true);
    expect(smokeResults.results.length).toBeGreaterThan(0);
    expect(smokeResults.assertionsCount).toBe(smokeResults.results.length);
    for (const item of smokeResults.results) {
      expect(item.ok).toBe(true);
    }

    const fullStdout = stdout.join('');
    const match = fullStdout.match(/✓ 冒烟通过：(\d+) 项断言全部成立/);
    expect(match).not.toBeNull();
    const stdoutAssertionCount = Number(match![1]);
    expect(stdoutAssertionCount).toBe(smokeResults.assertionsCount);
  });
});

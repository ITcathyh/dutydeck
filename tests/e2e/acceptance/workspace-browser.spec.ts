    ...
    const seedSession = async (promptText: string) => {
      ...
      while (Date.now() < deadline) {
        const [sessionRes, tasksRes] = await Promise.all([
          instance.request('GET', `/api/sessions/${session.id}`),
          instance.request('GET', `/api/sessions/${session.id}/tasks`)
        ]);
        const targetTask = Array.isArray(tasksRes.json)
          ? tasksRes.json.find((t: any) => t.id === taskId)
          : undefined;
        if (sessionRes.json?.state === 'idle' && targetTask?.status === 'completed') break;
        await new Promise(r => setTimeout(r, 200));
      }

      const finalSessionCheck = await instance.request('GET', `/api/sessions/${session.id}`);
      const finalTasksCheck = await instance.request('GET', `/api/sessions/${session.id}/tasks`);
      const targetTask = Array.isArray(finalTasksCheck.json)
        ? finalTasksCheck.json.find((t: any) => t.id === taskId)
        : undefined;
      expect(targetTask?.status).toBe('completed');
      expect(finalSessionCheck.json?.state).toBe('idle');

...
describe('Trae through ACPX registry acceptance', () => {
  ...
  it('7. renders tool calls collapsed and expandable in the Web UI', async () => { const source = await readFile(resolve(process.cwd(), 'apps/web/src/App.tsx'), 'utf8'); expect(source).toContain('setOpen(value => !value)'); expect(source).toContain('<ToolCard'); });
  it('8. removes Agent permission choices from the Web UI', async () => { const source = await readFile(resolve(process.cwd(), 'apps/web/src/App.tsx'), 'utf8'); expect(source).not.toContain('PermissionModeControl'); expect(source).not.toContain('始终允许'); expect(source).toContain('当前版本固定使用完全访问'); });

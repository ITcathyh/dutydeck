...
describe('Trae through ACPX registry acceptance', () => {
  ...
  it('8. removes Agent permission choices from the Web UI', async () => { const app = await readFile(resolve(process.cwd(), 'apps/web/src/App.tsx'), 'utf8'); const permissionCard = await readFile(resolve(process.cwd(), 'apps/web/src/components/PermissionCard.tsx'), 'utf8'); expect(app).not.toContain('PermissionModeControl'); expect(app).not.toContain('始终允许'); expect(permissionCard).not.toContain('PermissionModeControl'); expect(permissionCard).not.toContain('始终允许'); expect(permissionCard).toContain('当前版本固定使用完全访问'); });

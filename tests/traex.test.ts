...
describe('Trae through ACPX registry acceptance', () => {
  ...
  it('2-5. starts the configured ACP process, initializes, creates a session, and sends a prompt', async () => {
    ...
    const runtime = new DutydeckRuntime(repos, { probe: () => ({ protocol: 'acp', available: true, pause: false, resume: true }) }); await runtime.initialize([trae]);
  ...
  it('12. reports an explicit dependency error when the CLI disappears', async () => { const repos = createRepositories(':memory:'); const runtime = new DutydeckRuntime(repos, { probe: () => ({ protocol: 'acp', available: false, detail: 'CLI unavailable', pause: false, resume: true }) }); await runtime.initialize([traeAgent(cwd)]); await expect(runtime.start({ agentId: 'trae' })).rejects.toMatchObject({ code: 'AGENT_UNAVAILABLE' }); repos.close(); });
});
